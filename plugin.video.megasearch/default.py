...
#            addDir('[COLOR red][DOWN][/COLOR] [COLOR gold]TRAILERS[/COLOR]','file:///%s\l.m3u'%addonDir,4559,'')
            ...
            addDir('[COLOR gold]Emulators & Roms[/COLOR]','s',644,'http://icons.iconseeker.com/png/fullsize/fresh-addon/part.png')
            ...
            addDir('[COLOR brown]Link Tester[/COLOR]','f',1457,'http://icons.iconseeker.com/png/fullsize/nova/test-tubes.png')
            addDir('[COLOR brown]View Scan History[/COLOR]','file:///%s\downloads.txt'%addonDir,1458,'http://icons.iconseeker.com/png/fullsize/toolbar-icons-6/favorites-7.png')
            addDir('[COLOR brown]View Saved CCams[/COLOR]','file:///%s\cam.txt'%addonDir,1458,'http://icons.iconseeker.com/png/fullsize/toolbar-icons-6/favorites-7.png')
def MISC(url):
        ...
        try:
            try:
                ...
                if dp.iscanceled(): break;
                   ...
                   dp.update(percent,'[B]Testing Link[/B] - [COLOR yellow]%s[/COLOR] [B]Result[/B] - [COLOR black]TIMED OUT[/COLOR]'%(count))
                   if dp.iscanceled(): break;
        except:
            pass
...
def roms():
            addDir('easy-rom.com','http://www.easy-rom.com/roms/',8,'ww')
            addDir('youknowhim.com','http://youknowhim.com/Roms/',8,'ww')
...
def openSettings():
    ADDON.openSettings()
def saveurl(name,url):
        LF = open('%s/Links.txt'%addonDir, 'a')
        LF.write('#EXTINF:-1,'+name+'\n')
        LF.write(url+'\n')
        LF.close()
def savecam(name,url):
        LF = open('%s/cam.txt'%addonDir, 'a')
        LF.write(name+'\n')
        LF.write(url+'\n')
        LF.close()
...
def FTP(url):
        ...
        for url2,name in match:
            ...
            if find_this6 in str1:
                addDir2('[COLOR green][PLAY][/COLOR] %s'%name,url+url2,10,'')
            if find_this7 in str1:
                addDir2('[COLOR blue][View Streams][/COLOR] %s'%name,url+url2,10,'')
            if find_this8 in str1:
                addDir2('[COLOR pink][READ][/COLOR] %s'%name,url+url2,977,'')
            if find_this9 in str1:
                addDir2('[COLOR red][VIEW][/COLOR] %s'%name,url+url2,10,'')
            if find_this10 in str1:
                addDir2('[COLOR red][DOWNLOADS][/COLOR] %s'%name.replace('..&gt;',''),url+url2,4562,'')
            if find_this11 in str1:
                addDir2('[COLOR red][DOWNLOADS][/COLOR] %s'%name.replace('..&gt;',''),url+url2,4562,'')
            if find_this12 in str1:
                addDir2('[COLOR red][DOWNLOADS][/COLOR] %s'%name.replace('..&gt;',''),url+url2,4562,'')
            if find_this13 in str1:
                addDir2('[COLOR red][DOWNLOADS][/COLOR] %s'%name.replace('..&gt;',''),url+url2,4562,'')
            if find_this14 in str1:
                addDir2('[COLOR red][DOWNLOADS][/COLOR] %s'%name.replace('..&gt;',''),url+url2,4562,'')
            if find_this15 in str1:
                addDir2('[COLOR red][DOWNLOADS][/COLOR] %s'%name.replace('..&gt;',''),url+url2,4562,'')
            if find_this16 in str1:
                addDir2('[COLOR red][DOWNLOADS][/COLOR] %s'%name.replace('..&gt;',''),url+url2,4562,'')
            if find_this17 in str1:
                addDir2('[COLOR red][DOWNLOADS][/COLOR] %s'%name.replace('..&gt;',''),url+url2,4562,'')
...
def DOWNLOAD2(name,url):
    ...
    if dialog.yesno("DOWNLOAD FILE", 'Do you Wish To Download %s'%file,'', "",'Close','Yes'):
        file = os.path.basename(url)
        DownloaderClass(url,addonDir+'/resources/downloads/'+file) 
    else:
        return 
...
def addDir2(name,url,mode,iconimage):
        ...
        menu.append(('[COLOR gold]SAVE LINK[/COLOR]','Container.Refresh(%s?mode=4563&name=%s&url=%s)'% (sys.argv[0],name,url)))
        menu.append(('[COLOR gold]SAVE CAMLINE[/COLOR]','Container.Update(%s?mode=4564&name=%s&url=%s)'% (sys.argv[0],name,url)))
        menu.append(('[COLOR gold]DOWNLOAD[/COLOR]','Container.Update(%s?mode=4565&name=%s&url=%s)'% (sys.argv[0],name,url)))
        liz=xbmcgui.ListItem(name, iconImage="DefaultFolder.png", thumbnailImage=iconimage)
...
def addDir4(name,url,mode,iconimage):
        ...
        menu.append(('REFRESH', 'Container.Refresh'))        
        menu.append(('[COLOR gold]SAVE LINK[/COLOR]','Container.Refresh(%s?mode=4563&name=%s&url=%s)'% (sys.argv[0],name,url)))
        menu.append(('[COLOR gold]SAVE CAMLINE[/COLOR]','Container.Refresh(%s?mode=4564&name=%s&url=%s)'% (sys.argv[0],name,url)))
        menu.append(('[COLOR gold]DOWNLOAD[/COLOR]','Container.Refresh(%s?mode=4565&name=%s&url=%s)'% (sys.argv[0],name,url)))
        liz.addContextMenuItems(items=menu, replaceItems=False,)
